...
for o in range(len(i)):
    print('%.1f & %.1f $\pm$ %.1f \\\\' % (i[o], r[o], sig[o]))
    '''
=-=-=-=-=
Values
=-=-=-=-=
...
ax2.plot(x_,func2(x_,*thetahat2),color='tab:blue',label='fit')
...
ax3.plot(i_,r_,'o',color='black',label='data')
ax3.errorbar(i_,r_,yerr=sig_,fmt='o',color='black')
ax3.plot(x,func3(x,*thetahat3),color='tab:green',label='fit')

ax3.set_xlabel(r'$\theta_{i}$ $[Radians]$')
ax3.set_ylabel(r'$\theta_{r}$ $[Radians]$')
...
fig3.savefig("Plot 3.pdf", format='pdf')
